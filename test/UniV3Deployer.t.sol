...
contract Univ3Deployer_test is Test, Univ3Deployer {
  ...
  function _getPool(PoolAddress.PoolKey memory poolKey) internal view returns (IUniswapV3Pool pool) {
    return IUniswapV3Pool(PoolAddress.computeAddress(address(v3Factory), poolKey));
  }
  ...
  function test_create_pool() public {
    ...
    address pool = v3Factory.createPool(address(base), address(quote), 500);
    assertEq(pool, v3Factory.getPool(address(base), address(quote), 500));
